...
class Actor(object):
    ...
    def _build_net(self, scope):
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            hidden1 = tf.keras.layers.Dense(self.s_dim, activation='relu', use_bias=False, 
                                            kernel_regularizer=regularizers.l2(0.01))(state)
            batchNorm1 = tf.keras.layers.BatchNormalization(axis=-1)(hidden1)
            hidden2 = tf.keras.layers.Dense(self.s_dim, activation='relu', use_bias=False,                         
                                            kernel_regularizer=regularizers.l2(0.01))(batchNorm1)
            batchNorm2 = tf.keras.layers.BatchNormalization(axis=-1)(hidden2)                                 # (1, 360)
            output = tf.keras.layers.Dense(self.a_dim, activation='tanh', use_bias=False)(batchNorm2)
        return state, output
...
class Critic(object):
    ...
    def _build_net(self, scope):  # DQN architechture  
        with tf.compat.v1.variable_scope(scope):
            ...
            layer1 = tf.keras.layers.Dense(self.s_dim + self.a_dim, activation=tf.nn.relu, use_bias=False,
                                           kernel_regularizer=regularizers.l2(0.01))(inputs)
            layer2 = tf.keras.layers.Dense(32, activation=tf.nn.relu, use_bias=False,
                                           kernel_regularizer=regularizers.l2(0.01))(layer1)
