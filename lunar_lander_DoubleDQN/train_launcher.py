...
def learn_from_batch(replay_buffer, dqn_online, dqn_target, batch_size, gamma, s_dim, a_dim, tau):
    ...
    targets = reward_batch + gamma*(np.amax(dqn_target.predict_on_batch(next_state_batch), axis=1))*(1-done_batch)  # Q(s,a) (32,)
    targets_full = dqn_online.predict_on_batch(state_batch)   #(32, 4)
    targets_full = targets_full.numpy()   # Convert tensor to numpy object
    idx = np.arange(batch_size)
    targets_full[[idx], [action_batch]] = targets  # assignment

    loss = dqn_online.train_on_batch(state_batch, targets_full)
    for t, o in zip(dqn_target.trainable_variables, dqn_online.trainable_variables):
        t.assign(t * (1 - tau) + o * tau)
    return loss



def train(env, dqn_online, dqn_target, s_dim, a_dim, args):
    ...
    for i in range(int(args['max_episodes'])):
        ...
        for j in range(args['max_episodes_len']):
            ...
            if np.random.rand() > epsilon:
                q_value = dqn_online.predict(np.reshape(state, [1, s_dim]))
        ...
        end_time = time.time()
        print(f"========== Episode {i+1}, Total {j+1} Rounds,  Reward {ep_reward:.3f}, Loss {ep_loss:.3f}, Time {end_time - start_time:.3f}  =========")
...
def main(args):
    ...
    a_dim = 4
    # deep neural network
    dqn_online = DQN(s_dim, a_dim, int(args['batch_size']), float(args['lr']))._build_net()
    dqn_target = DQN(s_dim, a_dim, int(args['batch_size']), float(args['lr']))._build_net()
    train(env, dqn_online, dqn_target, s_dim, a_dim, args)
    # save model
    dqn_online.save('double_dqn.h5')
    env.close()
